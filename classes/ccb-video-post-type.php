...
if ( ! class_exists( 'CCB_Video_Post_Type' ) ) {
	...
	class CCB_Video_Post_Type extends CCB_Module implements CCB_Custom_Post_Type {
		...
		public static function markup_meta_boxes( $post, $box ) {
			...
			switch ( $box['id'] ) {
				case 'ccb_video-url-box':
					$variables['video_url'] = get_post_meta( $post->ID, 'ccb_video-url', true );
			...
			echo self::render_template( $view, $variables ); // XSS okay.
		}
		...
		public static function save_post( $post_id, $revision ) {
			...
			if ( isset( $_GET['action'] ) && in_array( $_GET['action'], $ignored_actions, true ) ) { // CSRF okay.
				return;
			}
			if ( ! $post || self::POST_TYPE_SLUG !== $post->post_type || ! current_user_can( 'edit_post', $post_id ) ) {
				return;
			}
			if ( ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) || 'auto-draft' === $post->post_status ) {
				return;
			}
			if ( check_admin_referer( 'ccb_video-url-nonce', '_ccb_video-url' ) && isset( $_POST['ccb_video-url'] ) ) {
				update_post_meta( $post_id, 'ccb_video-url', esc_url_raw( $_POST['ccb_video-url'] ) );
			}
		}
		...
		public static function cpt_shortcode_example( $attributes ) {
			...
			return self::render_template(
				'wpps-cpt-example/shortcode-cpt-shortcode-example.php', array(
		...
		public function register_hook_callbacks() {
			...
			add_action( 'post_updated', __CLASS__ . '::save_post', 10, 2 );
